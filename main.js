            ...
            $('#' + index).change(function() {
                ...
                PS4.PauseProcess(SelectedProcess.pid, null, function()
                {
                    if (memory.length !== undefined)
                    {
                        ...
                        if (activated)
                        {
                            PS4.Notify(222, name + ' | Activado ');
                        }
                        else
                        {
                            PS4.Notify(222, name + ' | Desactivado ');
                        }
                    }
                });
            });
...
function HandleTrainer(mods)
{
    ...
    if (SelectedProcess == null || typeof ProcessMaps.state == 'function' || typeof ProcessInfo.state == 'function') {
        $("#Message").text("Konexion del Trainer Falida");
        good = false;
    }
    ...
    if(good) {
        $("#Message").text("Trainer Konectado!");
        PS4.Notify(222, "Trainer Konectado!");
    }
...
GetRunningCUSA();
setTimeout(function(){if(sessionStorage.runningCUSA==null){gameSTAT.innerHTML="ID del Juego : <font color=#dc3545>Ninguno";}},2000);
...
if(trainerYN==0){
	alert("lo Siento -_- No hay Trainer para este Juego");
...
}).fail(function(){sessionStorage.WebRTE="no";rteSTAT.innerHTML="WebRTE : <font color=#dc3545>Sin Activar";});
...
PS4.GetProcessList(GetProcessListCallback, FailCallback);
	...
	var tid = pi.titleid.trim();
		if(tid.startsWith("CUSA")){
		sessionStorage.runningCUSA=tid;
		gameSTAT.innerHTML="ID del Juego : "+tid;
		setTimeout(SearchRunningGame,50);}
	});
});
}).fail(function(){sessionStorage.WebRTE="no";rteSTAT.innerHTML="WebRTE : <font color=#dc3545>Sin Activar";});
